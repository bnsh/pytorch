...
# Is there a better way to preinitialize a weight matrix?
			midparams = [x for x in self.middle.parameters()]
			midparams[0].data.copy_(torch.FloatTensor([-10,-10]))
			midparams[1].data.copy_(torch.FloatTensor([[5,-5],[5,-5]]))
			midparams[2].data.copy_(torch.FloatTensor([5,15]))
			midparams[3].data.copy_(torch.FloatTensor([0,0]))

			topparams = [x for x in self.top.parameters()]
			topparams[0].data.copy_(torch.FloatTensor([-5,5]))
			topparams[1].data.copy_(torch.FloatTensor([-5]))
